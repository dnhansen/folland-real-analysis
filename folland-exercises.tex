...
\begin{remarkbreak}[Completeness of uniform convergence on compact sets]
	Let $X$ be a $\sigma$-compact LCH space, and let $\rho$ be the metric from Exercise~56d on $\complex^X$ of uniform convergence on compact sets. We claim that $C(X)$ is complete in this metric.

	Let $(f_n)_{n\in\naturals}$ be a Cauchy sequence in $C(X)$, and let $(K_j)_{j\in\naturals}$ be an exhaustion of $X$ by compact sets. For any compact $K \subseteq X$ the sequence $(f_n|_K)$ is then Cauchy in $C(K)$ (equipped with the uniform metric), hence converges to a unique function $f_K$ by Proposition~4.13. Notice that $f_K = f_{K'}$ on $K \intersect K'$ for any other compact set $K'$. The function $f \colon X \to \complex$ given by $f(x) = f_K(x)$ if $x \in K$ is thus well-defined.

	Note that $f$ is continuous on $\interior{K_j}$, so since the $\interior{K_j}$ form an open cover of $X$, $f$ is continuous on $X$. And $f_n$ converges uniformly to $f$ on compact sets since any compact set is covered by finitely many $\interior{K_j}$.
\end{remarkbreak}
